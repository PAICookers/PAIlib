...
from .frame_defs import FrameFormatV2 as FFV2
from .frame_defs import FrameHeaderV2 as FHV2
from .frame_defs import OfflineControlFrame1Format as Off_CF1F
from .frame_defs import OfflineControlFrame2Format as Off_CF2F
from .frame_defs import OfflineControlFrame3Format as Off_CF3F
from .frame_defs import OfflineWorkFrame1FormatV2 as Off_WF1F_V2
from .frame_defs import OfflineWorkFrame2FormatV2 as Off_WF2F_V2
from .frames import *
from .types import *
...
)  # Config Frames; Work Frames; Control Frames
from .frame_defs import FrameHeaderV2 as FHV2
...
from .frame_defs import OfflineConfigFrame4FormatV2 as Off_Cfg4_V2
from .frame_defs import OfflineControlFrame1Format as Off_CF1F
from .frame_defs import OfflineControlFrame2Format as Off_CF2F
from .frame_defs import OfflineControlFrame3Format as Off_CF3F
from .frame_defs import OfflineWorkFrame1FormatV2 as Off_WF1F_V2
...
class OfflineFrameGenV2:
    ...
    def gen_core_config(
        ...
        w0 = (
            ...
            | (
                (params.get("neuron_number", 0) & 0x1FFF)
                << F.Word0.NEURON_NUMBER_OFFSET
            )
            | ((params.get("test_core_xy", 0) & 0x3F) << F.Word0.TEST_CORE_XY_OFFSET)
            | ((params.get("test_core_x", 0) & 0x3F) << F.Word0.TEST_CORE_X_OFFSET)
            | (
                (params.get("test_core_y_high", 0) & 0x3)
                << F.Word0.TEST_CORE_Y_HIGH2_OFFSET
            )
        )
        ...
        w1 = (
            (
                (params.get("test_core_y_low", 0) & 0xF)
                << F.Word1.TEST_CORE_Y_LOW4_OFFSET
            )
            | ((params.get("global_send", 0) & 0x7F) << F.Word1.GLOBAL_SEND_OFFSET)
            | ((params.get("csc_accelerate", 0) & 1) << F.Word1.CSC_ACCELERATE_OFFSET)
            | (
                (params.get("global_receive", 0) & 0x3F)
                << F.Word1.GLOBAL_RECEIVE_OFFSET
            )
        ...
        w2 = (
            ...
            | (
                (params.get("tick_duration", 0) & 0xFFFFFFFF)
                << F.Word2.TICK_DURATION_OFFSET
            )
    ...
    def gen_lut_config(
        ...
        for entry in lut_entries:
            w = ((entry.get("potential", 0) & 0xFFFFFFFF) << F.POTENTIAL_OFFSET) | (
    ...
    def gen_neuron_config(
        ...
        if mode == "full":
            ...
            for neu in neurons:
                ...
                w0 = (
                    (
                        (neu.get("tick_relative", 0) & 0xFF)
                        << F.Word0.TICK_RELATIVE_OFFSET
                    )
                    | ((neu.get("addr_axon", 0) & 0x1FF) << F.Word0.ADDR_AXON_OFFSET)
                    | (
                        (neu.get("addr_core_xy", 0) & 0x3F)
                        << F.Word0.ADDR_CORE_XY_OFFSET
                    )
                    | ((neu.get("addr_core_x", 0) & 0x3F) << F.Word0.ADDR_CORE_X_OFFSET)
                    | ((neu.get("addr_core_y", 0) & 0x3F) << F.Word0.ADDR_CORE_Y_OFFSET)
                    | (
                        (neu.get("addr_copy_xy", 0) & 0x3F)
                        << F.Word0.ADDR_COPY_XY_OFFSET
                    )
                    | ((neu.get("addr_copy_x", 0) & 0x3F) << F.Word0.ADDR_COPY_X_OFFSET)
                    | ((neu.get("addr_copy_y", 0) & 0x3F) << F.Word0.ADDR_COPY_Y_OFFSET)
                    | (
                        (neu.get("weight_skew_high", 0) & 0x7FF)
                        << F.Word0.WEIGHT_SKEW_HIGH_OFFSET
                    )
                )
                # Word 1
                w1 = (
                    (
                        (neu.get("weight_skew_low", 0) & 0x1F)
                        << F.Word1.WEIGHT_SKEW_LOW_OFFSET
                    )
                    | (
                        (neu.get("weight_addr_start", 0) & 0xFFF)
                        << F.Word1.WEIGHT_ADDRESS_START_OFFSET
                    )
                    | (
                        (neu.get("weight_addr_end", 0) & 0xFFF)
                ...
                w2 = (
                    ...
                    | (
                        (neu.get("thres_neg_mode", 0) & 1)
                        << F.Word2.THRESHOLD_NEG_MODE_OFFSET
                    )
                    | (
                        (neu.get("thres_pos_mode", 0) & 1)
                        << F.Word2.THRESHOLD_POS_MODE_OFFSET
                    )
                    | (
                        (neu.get("thres_neg", 0) & 0xFFFFFFFF)
                        << F.Word2.THRESHOLD_NEG_OFFSET
                    )
                    | (
                        (neu.get("thres_pos_hi", 0) & 0xFFF)
                        << F.Word2.THRESHOLD_POS_HI_OFFSET
                    )
                )
                # Word 3
                w3 = (
                    (
                        (neu.get("thres_pos_lo", 0) & 0xFFFFF)
                        << F.Word3.THRESHOLD_POS_LO_OFFSET
                    )
                    | (
                        (neu.get("lateral_inhibit", 0) & 1)
                        << F.Word3.LATERAL_INHIBITION_OFFSET
                    )
                    | (
                        (neu.get("leak_multi_seq", 0) & 1)
                        << F.Word3.LEAK_MULTI_SEQUENCE_OFFSET
                    )
                    | (
                        (neu.get("leak_multi_in", 0) & 1)
                        << F.Word3.LEAK_MULTI_INPUT_OFFSET
                    )
                    | (
                        (neu.get("leak_multi_mode", 0) & 1)
                        << F.Word3.LEAK_MULTI_MODE_OFFSET
                    )
                    | (
                        (neu.get("leak_add_mode", 0) & 1)
                    ...
                    | (
                        (neu.get("weight_compress", 0) & 1)
                        << F.Word3.WEIGHT_COMPRESS_OFFSET
                    )
                    | (
                        (neu.get("vjt_initial", 0) & 0xFFF)
            ...
            for neu in neurons:
                w0 = (
                    (
                        (neu.get("fold_range_xy", 0) & 0x7FF)
                        << G.Word0.FOLD_RANGE_XY_OFFSET
                    )
                    | (
                        (neu.get("fold_range_x", 0) & 0x7FF)
                        << G.Word0.FOLD_RANGE_X_OFFSET
                    )
                    | (
                        (neu.get("fold_range_y", 0) & 0x7FF)
                        << G.Word0.FOLD_RANGE_Y_OFFSET
                    )
                    | (
                        (neu.get("fold_skew_xy", 0) & 0x7FF)
                        << G.Word0.FOLD_SKEW_XY_OFFSET
                    )
                    | (
                        (neu.get("fold_skew_x", 0) & 0x7FF)
                        << G.Word0.FOLD_SKEW_X_OFFSET
                    )
                    | (
                        (neu.get("fold_skew_y_hi", 0) & 0x1FF)
                        << G.Word0.FOLD_SKEW_Y_HIGH_OFFSET
                    )
                )
                w1 = (
                    (
                        (neu.get("fold_skew_y_lo", 0) & 0x3)
                        << G.Word1.FOLD_SKEW_Y_LOW_OFFSET
                    )
                    | (
                        (neu.get("fold_axon_xy", 0) & 0x7FF)
                        << G.Word1.FOLD_AXON_XY_OFFSET
                    )
                    | (
                        (neu.get("fold_axon_x", 0) & 0x7FF)
                        << G.Word1.FOLD_AXON_X_OFFSET
                    )
                    | (
                        (neu.get("fold_axon_y", 0) & 0x7FF)
                        << G.Word1.FOLD_AXON_Y_OFFSET
                    )
                    | (
                        (neu.get("fold_number", 0) & 0x1FFFFFFF)
                        << G.Word1.FOLD_NUMBER_OFFSET
                    )
                )
                w2 = (
    ...
    def gen_data_frame(
        ...
        F = Off_WF1F_V2
        if timestep > F.TIMESTEP_MASK:
            raise ValueError("timestep overflow")
        if axon_addr > F.AXON_ADDR_MASK:
            raise ValueError("axon_addr overflow")
        if data > F.DATA_MASK:
    ...
    def gen_vjt_frame(
        ...
        if axon_addr > F.AXON_ADDR_MASK:
            raise ValueError("axon_addr overflow")
        if data_part > F.DATA_PART_MASK:
            raise ValueError("data_part overflow")
